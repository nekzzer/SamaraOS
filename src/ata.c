...
bool ata_init(void)                  { return probe_drive(&drives[0]); }
bool ata_present(void)               { return drives[0].present; }
uint32_t ata_total_sectors(void)     { return drives[0].sectors; }
int ata_read_sectors(uint32_t lba, int count, void* buf) {
    return ata_read(0, lba, count, buf);
}
